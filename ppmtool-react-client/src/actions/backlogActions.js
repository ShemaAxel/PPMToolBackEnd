...
export const addProjectTask = (
  ...
  try {
    await axios.post(`/api/backlog/${backlog_id}`, projet_task);
...
export const updateProjectTask = (
  ...
  try {
    await axios.patch(`/api/backlog/${backlog_id}/${task_id}`, projet_task);
    history.push(`/projectBoard/${backlog_id}`);
    dispatch({
      type: GET_ERRORS,
      payload: {}
...
export const deleteProjectTask = (backlog_id, task_id) => async dispatch => {
  if (window.confirm(`You ae deleting project task ${task_id}`)) {
